...
Проект представляет собой сравнительное тестирование для анализа трёх алгоритмов SSSP (Single-Source Shortest Path~--- поиск кратчайшего пути из одной вершины), реализованных с использованием библиотек SuiteSparse:GraphBLAS\footnote{URL: \url{https://github.com/DrTimothyAldenDavis/GraphBLAS} (дата обращения: 30.07.2026)} и LAGraph\footnote{URL: \url{https://github.com/DrTimothyAldenDavis/LAGraph} (дата обращения: 30.07.2026)} на языке C.

Алгоритм \textbf{Delta-Stepping} реализован через вызов функции из библиотеки LAGraph
...
Во время замеров был включён производительный режим, фоновые приложения отключены. Питание ноутбука было от сети с полным зарядом.
...
алгоритмам: каждая итерация выполняет последовательные вызовы
\texttt{GrB\_extractTuples} с последующим поэлементным обновлением расстояний
и приоритетной очереди, что не позволяет эффективно использовать векторизацию
GraphBLAS.
Оптимальное значение $\delta$ зависит от
структуры графа и не монотонно связано с плотностью: например, для roadNet-CA
(наименее плотный граф) $\delta = 10$ даёт лучший результат, тогда как для
com-Amazon $\delta = 0.5$ оказывается оптимальным.
...
\begin{itemize}
    ...
    \item проведены замеры производительности разработанных решений и готовой реализации из LAGraph: Algebraic Bellman-Ford в большинстве случаев уступает по производительности Delta-Stepping, реализованному в LAGraph, и сильно превосходит алгоритм Дейкстры.
\end{itemize}
